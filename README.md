...
| :--- | :--- |
| Real ESP / mailing queue | `message_outbox` table is the producer contract; swap the consumer for Redis + ESP dispatcher |
...
**Sprint 6 (next):** real ESP credentials when available, bulk lead import, per-rep activity SLA reporting (we capture `firstOutreachAt` but never surface compliance), and a "mark deal lost" flow with reason capture.
